		...
		if ( isset( $tool_callback['rest_alias'] ) ) {
			try {
                ...
                if ( $rest_response->is_error() ) {
                    $error = apply_filters('wordpress_mcp_rest_api_response_error', $rest_response->as_error(), $tool_name );
                    $error_code = apply_filters('wordpress_mcp_rest_api_response_error_code', McpErrorHandler::REST_API_ERROR, $rest_response->as_error(), $tool_name);
