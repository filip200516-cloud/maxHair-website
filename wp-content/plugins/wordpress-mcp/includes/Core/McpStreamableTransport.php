...
class McpStreamableTransport extends McpTransportBase {
	...
	private function handle_post_request( $request ) {
		try {
            ...
            if (!empty($response_body['error']['error'])) {
                $errorData = $response_body['error']['error'];

                $response_body['result'] = [
                    'content' => [
                        [
                            'type' => 'text',
                            'text' => 'Error: ' . ($errorData['data'] ?? $errorData['message'] ?? 'Unknown error')
                        ]
                    ],
                    'isError' => true
                ];

                unset($response_body['error']);
            }
